...
static void cdc_set_modem_state(usbd_device *dev, int iface, bool dsr, bool dcd) {
  ...
  usbd_ep_write_packet(dev, 0x84 + iface, buf, 10);
}
...
int _write(int file, char *ptr, int len) {
  if (file == STDOUT_FILENO || file == STDERR_FILENO) {
    usbd_ep_write_packet(usbd_dev, 0x85, ptr, len);
    return len;
  }
  errno = EIO;
  return -1;
...
void usb_cdc_setconfig(usbd_device *dev) {
  ...
  usbd_ep_setup(dev, 0x05, USB_ENDPOINT_ATTR_BULK, 64, usbuart_usb_out_cb);
  usbd_ep_setup(dev, 0x85, USB_ENDPOINT_ATTR_BULK, 64, usbuart_usb_in_cb);
